...
def diff_func_types(a, b):
    ...
    for i in a:
        ...
        if (len(diff) > 0):
            ...
            diff_change = diff
            set_diff = set(a[i])
    # print("diff_func_types" + address, diff_change, set_diff)
    return address, diff_change, set_diff
...
def func_updated(bv, function):
    ...
    try:
        ...
        # print("{}".format(str(bv.get_functions_containing(current_addr)[0])))
        # print("{}".format(str(function)))
    ...
    # if (bv.file.view == "Graph:PE" or bv.file.view == "Linear:PE"):
            if (eventfunc2 + 1 < time.time()):
                ...
                if (str(bv.get_functions_containing(current_addr)[0]) != str(function)):
                ...
                #Function Type Change (current function)
                # if (func_type != function.return_type):

                # if (function)
                # key_change, new_key_diff = diff_func_types(dict_funcs_new, dict_funcs)
                # if ()

                #Function name change (from a call)
                dict_funcs_new = func_types(bv)
                new_key_change, new_key_diff, new_set_diff = diff_func_types(dict_funcs_new, dict_funcs)
                if (new_key_diff > 0):
                ...
                # if (func_name != function.symbol.name):
                    ...
                    print('[*] Updating function name {name}'.format(name=function.symbol.name))
                    ...
                    if (old_key_change != 0 and new_key_change != 0):
                        ...
                        # Name change
                        if (function.symbol.name == str(new_key_diff)[6:-3]):
                            print("**** Name updated****")
                ...
                if (comment_state != function.comments):
                    ...
                    print("function.comments: {}".format(function.comments))
                    # Added
